...
namespace TextFileParserConsole
{
    class Program
    {
        ...
        {
            Data foo = new Data();
            foo.Header = new Header();
            foo.Header.AddColumn(new Column("FirstName","FirstName", typeof(string).Name));
            var row = new Row();
            row.AddValue("FirstName", "Darren");
            foo.Rows.Add(row);
            var fooarray = foo.ToSystemTextJson();
            var foostring = Encoding.Default.GetString(fooarray);
            var fooback = Data.FromSystemTextJson(fooarray);
            var foobar = true;
        }
        //static void Main(string[] args)
        //{
